  ...
  Future<bool> loginUser(Map<String, dynamic> user) async {
    try {
      ...
      QuerySnapshot querySnapshot =
          await users.where('email', isEqualTo: user['email']).where('password', isEqualTo:user['password']).get();
      String? userId;
      String? userEmail;
      String? userPassword;
      bool? isDelete;
      ...
      log('Error: ${e.toString()}', name: 'loginUser');
      Fluttertoast.showToast(
   ...
   /// update fields in a document in firebase
    ...
    Future<Map<dynamic, dynamic>> retrieveUserDataWithID(String userId) async {
      ...
      try {
        ...
        log("User id($userId) does not exist $e",
            name: 'retrieveUserDataWithID');
