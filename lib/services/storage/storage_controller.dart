...
class StorageController extends GetxController {
  /// access the AuthRepo instance from anywhere within the app
  ...
  Future<void> removeProfilePhotoFromFirebaseStorage(String documentId) async {
    try {
      // Delete the profile photo from Firebase storage
      var ref = await _storage
          .ref()
          .child('driverDetails/profilePhoto/$documentId/')
          .listAll();
  ...
  Future<void> addProfilePhototoFirebaseStorage(
    ...
    try {
      ...
      UploadTask task = _storage
          .ref('userProfile/profilePhotos/$documentId/$fileName')
          .putFile(addedImage);
      ...
      return task.then((snapshot) async {
        try {
          ...
          log("Profile picture added to Firebase Storage");
        } catch (e) {
